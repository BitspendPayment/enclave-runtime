...
impl Nsm for HostEntropy {
    ...
    /// PCRs are a property of a real enclave. There is nothing to report,
    /// extend or lock, so all three refuse rather than inventing a register
    /// file that would let boot logic appear to work outside an enclave.
    ...
    fn lock_pcr(&self, _index: u16) -> Result<()> {
        anyhow::bail!("no PCRs to lock: entropy is coming from the kernel, not an NSM")
    }
